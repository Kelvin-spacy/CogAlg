...
def form_Pm_(P_dert_):  # initialization, accumulation, termination
    ...
    for dert in P_dert_[1:]:
        ...
        if sign != _sign:  # sign change, terminate P
            P_.append(CP(sign=_sign, layer0=Clayer(L=L,I=I,D=D,M=M), dert_=dert_, sub_layers=sub_H, smP=False, fdert=False))
    ...
    P_.append(CP(sign=_sign, layer0=Clayer(L=L,I=I,D=D,M=M), dert_=dert_, sub_layers=sub_H, smP=False, fdert=False))  # incomplete P
    return P_
...
def form_adjacent_M_(Pm_):  # compute array of adjacent Ms, for contrastive borrow evaluation
    ...
    pri_M = Pm_[0].layer0.M  # comp_g value is borrowed from adjacent opposite-sign Ms
    M = Pm_[1].layer0.M
    adj_M_ = [abs(Pm_[1].layer0.M)]  # initial next_M, no / 2: projection for first P, abs for bilateral adjustment
...
def intra_Pm_(P_, adj_M_, fid, rdn, rng):  # evaluate for sub-recursion in line Pm_, pack results into sub_Pm_
    ...
    for P, adj_M in zip(P_, adj_M_):  # each sub_layer is nested to depth = sub_layers[n]
        ...
        if P.sign:  # +Pm: low-variation span, eval comp at rng=2^n: 2, 4., kernel: 5, 9., rng=1 cross-comp is kernels 2 and 3
            if P.layer0.M - adj_M > ave_M * rdn and P.layer0.L > 4:  # reduced by lending to contrast: all comps form params for hLe comp?
            ...
            if min(-P.layer0.M, adj_M) > ave_D * rdn and P.layer0.L > 3:  # cancelled M+ val, M = min | ~v_SAD

                rel_adj_M = adj_M / -P.layer0.M  # for allocation of -Pm' adj_M to each of its internal Pds
...
def intra_Pd_(Pd_, rel_adj_M, rdn, rng):  # evaluate for sub-recursion in line P_, packing results in sub_P_
    ...
    for P in Pd_:  # each sub in sub_ is nested to depth = sub_[n]
        if min(abs(P.layer0.D), abs(P.layer0.D) * rel_adj_M) > ave_D * rdn and P.layer0.L > 3:  # abs(D) * rel_adj_M: allocated adj_M
